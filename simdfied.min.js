...
Math.pow(c[k]-f[k],2);if(0==h||g<d)d=g,a.kmeansIdx[b]=h}for(h=0;h<a.settings.kmeansK;h++)if(b=a.kmeansIdx.map(function(a,b){return a==h?b:null},this).filter(function(a){return null!=a},this),0<b.length)for(k=0;k<a.n;k++)a.kmeansCentroids[h][k]=e.vec(a.X[k]).find(b).mean()}},ea=function(a){if(a&&a.isSimdfiedMl)return a;this.ml=a?a:{split:[1,0,0],settings:{addInter:!0,normMeanStd:!0,regLambda:!1,iter:100,alpha:.01,kmeansK:3,kmeansIter:10},worker:null};this.isSimdfiedMl=!0;this.algo=function(a){if("string"!=
...
a;return e.ml(this.ml)};this.set=function(a,c){if(!a||"string"!=typeof a||0==a.length)throw"setting key must be a non empty string";if(-1==["number","boolean"].indexOf(typeof c))throw"setting value must be a boolean or a number";this.ml.settings[a]=c;return e.ml(this.ml)};this.parse=function(a){return e.ml(JSON.parse(a,function(a,b){return"Infinity"===b?Infinity:"-Infinity"===b?-Infinity:"NaN"===b?NaN:b}))};this.reset=function(){switch(this.ml.algo){case "kmeans":D(this.ml)}};this.run=function(a){if(!this.ml.worker){var c=
"importScripts('"+document.location.href.substring(0,document.location.href.lastIndexOf("/"))+"/simdfied.min.js');onmessage = function (e){postMessage(simdfied.ml().parse(e.data).runWorker().stringify()); self.close()}";this.ml.worker=this.ml.worker||new Worker(window.URL.createObjectURL(new Blob([c])))}this.ml.worker.onmessage=function(c){c.data&&c.data.prog&&c.data.onProg?eval(c.data.onProg+"('"+c.data.prog+"');"):a(e.ml().parse(c.data))};this.ml.worker.postMessage(this.stringify())};this.runWorker=
function(){if(!this.ml.algo)throw'algorithm type must be specified using the "algo" function';if(!this.ml.X)throw'X must be specified using the "X" function';var a=v.indexOf(this.ml.algo);if(!this.ml.y&&-1!=["linReg","logReg"].indexOf(this.ml.algo))throw"y must be specified for "+B[a]+' using the "y" function';console.log("running "+B[a]);switch(this.ml.algo){case "linReg":case "logReg":ca(this.ml);a=this.ml;timeStamp=new Date;a.theta=e.vec().zeros(a.XNorm.length).vec;a.cost=[];a.cost[0]=C(a);console.log("initial cost: "+
m(a.cost[0].toFixed(2)));for(var c=1;c<a.settings.iter+1;c++){var d=q(a),d=e.vec(d).sub(a.yNorm),d=e.mat(a.XNorm).mulVec(d);a.theta=e.vec(a.theta).sub(d.mulScal(a.settings.alpha/a.m)).vec;d=C(a);if(-1!=[Infinity,-Infinity,NaN].indexOf(d)||d>a.cost[c-1])break;else a.cost[c]=d;postMessage&&postMessage({prog:c/a.settings.iter,onProg:a.onProg})}console.log("gradientDescent done ("+m(new Date-timeStamp)+"ms)");console.log("cost after #"+a.cost.length+" iterations "+m(a.cost[c-1].toFixed(2)));a=this.ml;
switch(a.algo){case "logReg":c=q(a),c=e.vec(c).sigmoid().round(),a.trainAcurr=e.vec(a.yNorm).comp(c).sum()/e.vec(a.yNorm).length(),console.log("train acurracy: "+100*a.trainAcurr+"%")}if(0<a.testSize)switch(a.algo){case "logReg":c=e.mat(a.XTestT).mulVec(a.theta),c=e.vec(c).sigmoid().round(),a.testAcurr=e.vec(a.yTest).comp(c).sum()/e.vec(a.yTest).length(),console.log("test acurracy: "+(100*a.testAcurr).toFixed(1)+"%")}break;case "kmeans":da(this.ml)}return e.ml(this.ml)};this.stringify=function(){return JSON.stringify(this.ml,
function(a,c){return"number"==typeof c&&isNaN(c)?"NaN":Infinity===c?"Infinity":-Infinity===c?"-Infinity":c})};this.hipo=function(){return q(this.ml)};this.predOne=function(a){var c=this.ml;if(-1==["linReg","logReg"].indexOf(c.algo))throw"prediction is not available for this algorithm";var d=a.slice();if(c.settings.normMeanStd)for(var h=0;h<d.length;h++)d[h]=(d[h]-c.mu[h])/c.sigma[h];c.settings.addInter&&d.unshift(1);d=e.vec().fromArray(d).mul(c.theta);"logReg"==c.algo&&(d=d.sigmoid());switch(c.algo){case "linReg":console.log("predicting for features ["+
